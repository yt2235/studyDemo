...
export function CategoryShowcase({ categories, products, locale, dict }: Props) {
    ...
    const [isL1Sticky, setIsL1Sticky] = useState(false);
    const l1SentinelRef = useRef<HTMLDivElement>(null);

    // Measurement logic for inline "Show All"
    useEffect(() => {
        const checkLayout = () => {
    ...
    useEffect(() => {
        const observer = new IntersectionObserver(
            ([entry]) => {
                setIsL1Sticky(!entry.isIntersecting);
            },
            { threshold: [0], rootMargin: '-65px 0px 0px 0px' }
        );
        if (l1SentinelRef.current) observer.observe(l1SentinelRef.current);
        return () => observer.disconnect();
    }, []);

    // Update activeL1 if it's empty but l1Cats is populated
    useEffect(() => {
        if (!activeL1 && l1Cats.length > 0) {
            setActiveL1(l1Cats[0].id);
        }
    }, [activeL1, l1Cats]);
    ...
    const validCategoryIds = useMemo(() => {
        if (!activeDeepestCategoryId) return [];
    ...
    return (
        <section id="products" className="pb-24 md:pb-32 scroll-mt-20  focus:outline-none">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
                ...
                <div className="text-sm font-medium text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-900 px-4 py-2 rounded-full border border-zinc-200 dark:border-zinc-800 shadow-sm flex items-center gap-2">
            ...
            {categories.length === 0 ? (
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div ref={l1SentinelRef} className="h-px w-full pointer-events-none" />
                    <div className="sticky top-12 z-40 transition-all duration-300">
                        <div className={`bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 rounded-2xl shadow-sm dark:shadow-black/20 transition-all duration-300 ${isL1Sticky ? 'p-2 md:p-2.5 rounded-t-none border-t-0 shadow-md' : 'p-3 md:p-4'}`}>
                            <h3 className={`text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest transition-all duration-300 ${isL1Sticky ? 'max-h-0 opacity-0 mb-0 overflow-hidden' : 'mb-3 opacity-100'}`}>
                    ...
                    <div className="flex flex-col lg:flex-row gap-6 md:gap-8 items-start">
                                    ...
                                    <div className="flex flex-col gap-4">
                                        ...
                                        <div className="flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0 no-scrollbar">
                                            ...
                                            <button
                                                ...
                                                className={`whitespace-nowrap text-left px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 flex items-center justify-between shrink-0 ${activeL2 === null
                                                ...
                                                return (
                                                    <div key={l2.id} className="flex flex-col gap-1 shrink-0">
                                                        <button
                                                            ...
                                                            className={`whitespace-nowrap text-left px-5 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 flex items-center justify-between gap-3 group ${isActive
                                                                ? "bg-blue-600 text-white shadow-lg shadow-blue-500/25"
                                                        ...
                                                        >
                                                            ...
                                                            {categories.some(c => c.level === 3 && c.parent_id === l2.id) && (
                                                                <svg
                                                                    className={`w-3.5 h-3.5 transition-transform duration-300 hidden lg:block ${isActive ? 'rotate-90 text-blue-200' : 'text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-200'}`}
                                                        ...
                                                        {isActive && currentL3s.length > 0 && (
                                                            <div className="hidden lg:block pl-4 mt-1 mb-2">
                                                                <div className="border-l border-zinc-200 dark:border-zinc-800 pl-3 flex flex-col gap-1 py-1">
                                                                    <button
                                                                        onClick={() => setActiveL3(null)}
                                                                        className={`text-left px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${activeL3 === null
                                                                    ...
                                                                    {currentL3s.map((l3) => (
                                                                        <button
                                                                            ...
                                                                            className={`text-left px-3 py-2 rounded-lg text-xs font-medium transition-colors ${activeL3 === l3.id
                                        ...
                                        {activeL2 && currentL3s.length > 0 && (
                                            <div className="lg:hidden flex flex-col gap-2 pt-3 border-t border-zinc-100 dark:border-zinc-800/50">
                                                <div className="flex flex-row gap-2 overflow-x-auto pb-1 no-scrollbar p-1">
                                                    <button
                                                        onClick={() => setActiveL3(null)}
                                                        className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-semibold transition-all shrink-0 ${activeL3 === null
                                                    ...
                                                    {currentL3s.map((l3) => (
                                                        <button
                                                            key={l3.id}
                                                            onClick={() => setActiveL3(l3.id)}
                                                            className={`whitespace-nowrap px-4 py-2 rounded-xl text-xs font-semibold transition-all shrink-0 ${activeL3 === l3.id
                                                                ? "bg-blue-600 text-white shadow-md shadow-blue-500/20 border border-blue-500"
                                                                : "bg-zinc-50 dark:bg-zinc-800/50 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 border border-zinc-100 dark:border-zinc-800/50"
                                                                }`}
                                                            title={l3.name}
                        ...
                        <div className="flex-1 w-full outline-none focus:outline-none">
                            {displayedProducts.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 outline-none focus:outline-none">
                                    {displayedProducts.map((product) => (
                                        <a
                                            href={`/${locale}/product/${product.id}`}
                                            key={product.id}
                                            className="group flex flex-col bg-white dark:bg-zinc-900/80 rounded-2xl border border-zinc-200/60 dark:border-zinc-800/80 overflow-hidden transition-all duration-500 hover:shadow-xl hover:shadow-blue-500/10 hover:-translate-y-1 hover:border-blue-300 dark:hover:border-blue-700/50 cursor-pointer"
                                        >
                                            {/* Image */}
                                            <div className="aspect-[4/3] relative overflow-hidden bg-zinc-50 dark:bg-zinc-950/50 p-3">
                                ...
                                <div className="p-16 flex flex-col items-center justify-center bg-white/40 dark:bg-zinc-900/40 rounded-3xl border border-zinc-200/50 border-dashed dark:border-zinc-800/50 backdrop-blur-sm">
                                    ...
                                    <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1 max-w-sm text-center">
                                        Try adjusting your subcategory filters or checking a different main category.
                                    </p>
                                </div>
                            )}
                        </div>
